...
class FlamingoProcessor:
    ...
    def __init__(
        ...
        self.tokenizer = AutoTokenizer.from_pretrained('facebook/opt-30b', use_fast=use_fast, padding_side='left')
        logging.info(f"DBGR FlamingoProcessor model_max_length: {self.tokenizer.model_max_length}")
    ...
    def _remove_tags(self, text: str) -> str:
        for s in ('<image>', '<EOC>', '</s>', self.tokenizer.eos_token, self.eoc_token, self.tokenizer.pad_token):
            text = text.replace(s, '')
        return text.strip()
