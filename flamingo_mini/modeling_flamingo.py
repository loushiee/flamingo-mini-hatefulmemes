...
class FlamingoBaseModel(ABC, PreTrainedModel):
    ...
    def freeze_vm(self):
        """freeze vision model """
        logging.info("DBGR FlamingoBaseModel freeze_vm")
        for param in self.vision_encoder.parameters():
            param.requires_grad = False
    
    def unfreeze_vm(self):
        """unfreeze vision model """
        logging.info("DBGR FlamingoBaseModel unfreeze_vm")
        for param in self.vision_encoder.parameters():
    ...
    def freeze_lm(self):
        ...
        """
        logging.info("DBGR FlamingoBaseModel freeze_lm")
    ...
    def unfreeze_lm(self):
        logging.info("DBGR FlamingoBaseModel unfreeze_lm")
        for param in self.lm.parameters():
            param.requires_grad = True
        ...
        **kwargs
        ...
        if visual_features is None:
            if xattn_past_key_values is None and pixel_values is not None:
                ...
                #logging.info("DBGR forward: visual_features is None!")
        ...
        if media_locations is None:
            logging.info("DBGR forward: media locations is None!")
            media_locations = torch.zeros(size=(batch_size, seq_length), dtype=torch.int, device=device)
        # else:
        #     logging.info(f"DBGR media locations '{media_locations}'")
...
class FlamingoOPT(FlamingoBaseModel):
    ...
    def __init__(self, config: FlamingoConfig):
        ...
        logging.info(f"DBGR OPT vocab_size: {base_lm.config.vocab_size}")
...
class FlamingoModel(PreTrainedModel):
    ...
    def predict_memes_hatefulness(
        ...
        **kwargs
        ...
        """
        helper utility for evaluating hatefulness of meme.
        """
        if device is None:
            device = self.device
        ...
        if images is not None:
            ...
            pixel_values = processor(images=images, device=device, max_length=max_length)['pixel_values']

        assert pixel_values is not None, "you must pass either images or visual features to generate_captions()!"
        assert prompts is not None, "Need to pass meme texts as prompts"
        assert gts is not None, "Need to pass meme ground truth texts"

        gts_ids, media_locations, attention_mask = processor.encode_text(gts, device)
        ...
        prompts_ids, media_locations, attention_mask = processor.encode_text(prompts, device)
        ...
        scores = list(gen_out.scores)

        # logging.info(f"DBGR gen_ids\n{gen_ids[:5]}")

        B = len(prompts)

        # Perform class prediction
        gens = processor.tokenizer.batch_decode(gen_ids, skip_special_tokens=True)
        # logging.info(f"DBGR gens\n{gens[:5]}")

        # Compute normalized log probabilities of answer token position for the classname tokens
        transition_scores = self.compute_transition_scores(gen_ids, scores, normalize_logits=True)
        # logging.info(f"DBGR transition_scores shape: {transition_scores.shape}\n{transition_scores}")

        token_ids, _, _ = processor.encode_text('on it. Is it hateful? Answer:', device)
        search_width = 6
        token_ids = token_ids.squeeze()[-search_width:]
        prompts_ids_max_len = prompts_ids.size(1)
        # logging.info(f"DBGR Is it hateful? Answer: IDs: {token_ids}")
        # logging.info(f"DBGR scores list length: {len(scores)}, element shape: {scores[0].shape} prompts_ids_max_len: {prompts_ids_max_len}")
        probs = []
        W = gen_ids.size(1)
        for i in range(B):
            for j in range(W):
                k = j+search_width
                if k < W:
                    if torch.equal(token_ids, gen_ids[i, j:k]):
                        # logging.info(f"DBGR Found answer token loc: {k}, token: {gen_ids[i,k]} word: '{processor.tokenizer.decode(gen_ids[i,k])}' gen_ids:\n{gen_ids[i]} text:'{gens[i]}'")
                        token = gen_ids[i,k]
                        answer = processor.tokenizer.decode(token)
                        if answer in ['yes', 'no']:
                            logit = transition_scores[i, k - prompts_ids_max_len]
                            prob = np.exp(logit.item())
                            if answer == 'no':
                                prob = 1 - prob
                            probs.append(prob)
                        else:
                            logging.info(f"DBGR token is not yes or no '{answer}'")
                            probs.append(0.5)
                        break
                else:
                    logging.info("DBGR not found answer token loc!!!")
                    probs.append(0.5)
                    break

        preds = []
        for g in gens:
            m = re.search(r'Is it hateful\? Answer:(yes|no)<EOC>', g, re.IGNORECASE)
            if m is None:
                logging.info(f"DBGR Wrong answer: {g}")
            p = (1 if m.group(1) == 'yes' else 0) if m else random.getrandbits(1)
            preds.append(p)

        return loss, preds, gens, probs
